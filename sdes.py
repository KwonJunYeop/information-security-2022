...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    if mode == 1:
        ...
        text_P = bitarray()
        for i in IP:
            text_P.append(text[i])

        # L, R 분리 후 R을 8bit로 확장
        text_L = text_P[0:4]
        text_R = text_P[4:8]

        EP_R = bitarray()
        for i in EP:
            EP_R.append(text_R[i])

        # 확장된 R과 schedule_keys의 결과 [0]을 round 함수로 보냄
        keys = schedule_keys(key)
        round_0 = round(EP_R, keys[0])
    ...
    # DECRYPT
        ...
        # 복호화이므로 round함수에 key 거꾸로 집어넣기
        # 확장된 R과 schedule_keys의 결과 [1]을 round 함수로 보냄
        keys = schedule_keys(key)
        round_0 = round(EP_R, keys[1])
